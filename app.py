...
if category == "각기둥/각뿔":
    ...
    n = st.sidebar.number_input("n각형", 3, 20, 3) # 기본값 3(삼각뿔)
    h, r = 4.0, 2.0
    if type_ == "각기둥": rt = r
...
for e, fs in edges.items():
    if len(fs) == 2:
        ...
        # 2. 원기둥 옆면 세로줄 삭제 (곡면일 때)
        if is_curved and np.dot(n1, n2) > 0.8:
             if v1 != v2: vis_edges.append(e) # 실루엣만 그림
...
# --- 5. 그리기 (여기가 핵심 수정사항입니다) ---
fig = go.Figure()

# [수정] 숨은 선: 회색(silver) -> 검정(black)으로 변경, 점선(dash) 적용
x_h, y_h, z_h = [], [], []
...
fig.add_trace(go.Scatter3d(
    ...
    line=dict(color='black', width=4, dash='dash'), # 진하고 굵은 점선
...
fig.add_trace(go.Scatter3d(
    ...
    line=dict(color='black', width=5), # 실선
